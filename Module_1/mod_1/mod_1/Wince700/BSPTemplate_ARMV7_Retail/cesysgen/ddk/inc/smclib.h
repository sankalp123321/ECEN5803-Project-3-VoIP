...
// By default, assume that this is the Windows CE version of smclib.h
#define SMCLIB_CE
#endif // !(defined(SMCLIB_VXD) || defined(SMCLIB_CE))
...
typedef struct _SCARD_READER_CAPABILITIES {
	...
	// Type of reader (Serial/USB/Keyboard etc)
	ULONG	ReaderType;
...
// To synchronize suspend/resume scenario
extern int g_IsResumed;
extern HANDLE g_hReEnumerationEvent;
...
__inline BOOL IsCallerKernelAndSetErrorIfRequired()
{
    const BOOL isCallerKernel =  (GetDirectCallerProcessId() == GetCurrentProcessId());
    if (!isCallerKernel)
    {
        DEB_0_A(1,L"Only the Kernel can IOCTL to smartcard drivers");
