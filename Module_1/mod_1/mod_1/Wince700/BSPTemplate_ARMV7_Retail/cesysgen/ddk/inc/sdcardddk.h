...
typedef enum _SD_SLOT_EVENT_TYPE {
    ...
    SDGetAdaptiveControl = 7,       // bus driver received a get adaptive control request
    SDSetAdaptiveControl = 8        // bus driver received a set adaptive control request
}SD_SLOT_EVENT_TYPE, *PSD_SLOT_EVENT_TYPE;

typedef struct _SD_ADAPTIVE_CONTROL
{
    BOOL    bAdapterRemoved;
    BOOL    bPwrCtrlOnSuspendResume;
    BOOL    bResetOnSuspendResume;
} SD_ADAPTIVE_CONTROL, *PSD_ADAPTIVE_CONTROL ;
...
typedef DWORD*  PSD_CARD_STATUS;     // ptr to card status
...
typedef struct _SD_FUNCTION_POWER_DRAW {
    ...
    USHORT      OpMaxPower;     // Maximum current when this function is operating
        ...
        //for SDIO 1.1 cards    Note: EMPC refers to the EMPC bit of the CCCR register "Power Control"
        //                            EPS  refers to the EPS  bit of the FBR  register for Power Selection
...
extern "C" {
    ...
    //  SDFreeBusRequest
    typedef SD_API_STATUS (*PSD_FREE_BUS_REQUEST) (PSD_BUS_REQUEST);
    ...
    typedef SD_API_STATUS (*PSD_IO_CHECK_HARDWARE)(SD_DEVICE_HANDLE, PSD_CARD_STATUS );

    typedef SD_API_STATUS (*PSD_IO_SET_ADAPTIVE_CONTROL)(SD_DEVICE_HANDLE, PVOID );
    typedef SD_API_STATUS (*PSD_IO_GET_ADAPTIVE_CONTROL)(SD_DEVICE_HANDLE, PVOID );
    ...
    typedef struct _SDCARD_API_FUNCTIONS {
        ...
        PSD_IO_CHECK_HARDWARE               pSDIOCheckHardware;
        PSD_IO_SET_ADAPTIVE_CONTROL         pSDIOSetAdaptiveControl;
        PSD_IO_GET_ADAPTIVE_CONTROL         pSDIOGetAdaptiveControl;
    ...
    VOID SDCardDebugOutput(TCHAR const*const pDebugText, ...);
...
#define SDGetCardStatusFromResponse(pCardResponse, pCardStatus) \
    ...
    //      returns WIN32 Error
    //
    DWORD SDGetRegPathFromInitContext(WCHAR const*const pActivePath, PWCHAR pRegPath, ULONG Length);
...
#define SDSetCardFeature(hDevice,         \
    ...
    //          typically in the completion callback
    //
#define SdBusRequestResponse(pRequest,pSdCmdResp) \
    g_SDClientApiFunctions.pSdBusRequestResponse(pRequest, pSdCmdResp)

    //  SDIOCheckHardware - checks the hardware status
    //  Input:  hDevice
    //  Output:
    //  Return:
    //  Notes:
#define SDIOCheckHardware(hDevice, pCardStatus) \
    g_SDClientApiFunctions.pSDIOCheckHardware((hDevice),(pCardStatus))

    //  SDIOSetAdaptiveControl - sets the adaptive control
    //  Inputs:  hDevice, pAdaptiveControl
    //  Output:
    //  Return:
    //  Notes:

#define SDIOSetAdaptiveControl(hDevice, pvAdaptiveControl) \
    g_SDClientApiFunctions.pSDIOSetAdaptiveControl((hDevice),(pvAdaptiveControl))

    //  SDIOGetAdaptiveControl - gets the adaptive control
    //  Inputs:  hDevice, pAdaptiveControl
    //  Output:
    //  Return:
    //  Notes:
#define SDIOGetAdaptiveControl(hDevice, pvAdaptiveControl) \
    g_SDClientApiFunctions.pSDIOGetAdaptiveControl((hDevice),(pvAdaptiveControl))
...
//
VOID SDOutputBuffer(__in_bcount(BufferSize) PVOID pBuffer, ULONG BufferSize);
