...
#define CD_PLAYSTATE_IDLE       0
#define CD_PLAYSTATE_PLAY       1
#define CD_PLAYSTATE_PAUSED     2
#define CD_PLAYSTATE_REQ_PAUSE  21
#define CD_PLAYSTATE_REQ_STOP   22

//
//  These media types are used internally only.
//
//  Bit 0   - ReWritable
//  Bit 1   - Sector addressable for write
//  Bit 2   - Defect managed
//  Bit 3   - DVD
//  Bit 4   - Incremental write
//  Bit 6   - Multitrack - media uses tracks/sessions.
//  Bit 7   - CD
//
//  There are no specific flags for MO,  since currently it requires no
//  special treatment (disk).  CD/DVD-R fall into the ROM category as
//  far as we're concerned.
//
//  Note that DVD+RW is sector addressable for write, but we don't treat
//  it like that.
//

#define MED_FLAG_REWRITE      0x00000001
#define MED_FLAG_SECTOR_ADDR  0x00000002
#define MED_FLAG_TDM          0x00000004
#define MED_FLAG_DVD          0x00000008
#define MED_FLAG_INCREMENTAL  0x00000010
#define MED_FLAG_MULTITRACK   0x00000040
#define MED_FLAG_CD           0x00000080
#define MED_FLAG_BG_FORMAT    0x01000000
#define MED_FLAG_MRW          0x02000000
#define MED_FLAG_DVD_PLUS     0x04000000
#define MED_FLAG_VERIFY_SUP   0x08000000

#define MED_TYPE_UNKNOWN 0
#define MED_TYPE_DISK         (MED_FLAG_REWRITE | MED_FLAG_SECTOR_ADDR | MED_FLAG_TDM)
#define MED_TYPE_DVD_ROM      (MED_FLAG_DVD)
#define MED_TYPE_CD_ROM       (MED_FLAG_CD)
#define MED_TYPE_CD_RW        (MED_FLAG_CD | MED_FLAG_REWRITE)
#define MED_TYPE_CD_R         (MED_FLAG_CD | MED_FLAG_INCREMENTAL)
#define MED_TYPE_CD_MRW       (MED_TYPE_DISK | MED_FLAG_CD | MED_FLAG_MRW | MED_FLAG_BG_FORMAT)
#define MED_TYPE_CD_RAM       (MED_TYPE_DISK | MED_FLAG_CD)
#define MED_TYPE_DVD_RAM      (MED_TYPE_DISK | MED_FLAG_DVD)
#define MED_TYPE_DVD_MINUS_RW (MED_FLAG_DVD | MED_FLAG_REWRITE)
#define MED_TYPE_DVD_PLUS_RW  (MED_TYPE_DVD_MINUS_RW | MED_FLAG_DVD_PLUS | MED_FLAG_BG_FORMAT)
#define MED_TYPE_DVD_PLUS_MRW (MED_TYPE_DVD_RAM | MED_FLAG_MRW | MED_FLAG_BG_FORMAT)
#define MED_TYPE_DVD_R        (MED_FLAG_DVD | MED_FLAG_INCREMENTAL)
#define MED_TYPE_DVD_PLUS_R   (MED_TYPE_DVD_R | MED_FLAG_DVD_PLUS)

#define MED_TYPE_ROM_MASK     (MED_FLAG_CD | MED_FLAG_DVD)
...
#define IOCTL_CDROM_SCAN_AUDIO       CTL_CODE(FILE_DEVICE_CD_ROM, 0x0020, METHOD_BUFFERED, FILE_ANY_ACCESS)
...
#define IOCTL_CDROM_READ_SG         CTL_CODE(FILE_DEVICE_CD_ROM, 0x0301, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_TEST_UNIT_READY CTL_CODE(FILE_DEVICE_CD_ROM, 0x0302, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_DISC_INFO       CTL_CODE(FILE_DEVICE_CD_ROM, 0x0303, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_TRACK_INFO      CTL_CODE(FILE_DEVICE_CD_ROM, 0x0304, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_SET_SPEED       CTL_CODE(FILE_DEVICE_CD_ROM, 0x0305, METHOD_BUFFERED, FILE_READ_ACCESS)

#define IOCTL_CDROM_GET_SENSE_DATA  CTL_CODE(FILE_DEVICE_CD_ROM, 0x0306, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_ISSUE_INQUIRY   CTL_CODE(FILE_DEVICE_CD_ROM, 0x0307, METHOD_BUFFERED, FILE_READ_ACCESS)

#define IOCTL_CDROM_GET_MEDIA_TYPE  CTL_CODE(FILE_DEVICE_CD_ROM, 0X0308, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_GET_PARTITION_INFO CTL_CODE(FILE_DEVICE_CD_ROM, 0X0309, METHOD_BUFFERED, FILE_READ_ACCESS)
#define IOCTL_CDROM_PAUSE_POLLING   CTL_CODE(FILE_DEVICE_CD_ROM, 0X030A, METHOD_BUFFERED, FILE_READ_ACCESS)

#define IOCTL_CDROM_UNLOAD_DRIVER        CTL_CODE(FILE_DEVICE_CD_ROM, 0x0402, METHOD_BUFFERED, FILE_READ_ACCESS)
...
typedef struct _TRACK_DATA_RAW
{
    UCHAR SessionNumber;
    UCHAR Control : 4;
    UCHAR Adr : 4;
    ...
    UCHAR PSEC;
    UCHAR PFRAME;
} TRACK_DATA_RAW, *PTRACK_DATA_RAW;
...
#define IOCTL_CDROM_ATAPI_CMD        0x07
...
//
// YellowMode2 is Mode 2 Formless Data - or 2336 bytes of user data.
// XAForm2 is Mode 2 Form 2 Data - or 2324 bytes of user data.
// CDDA is raw mode, using all 2352 bytes of the frame for data.
//
// Other than this, Mode 2 Form 1 and Mode 1 both use 2048 bytes of
// user data and are read compatible.  However, the two modes cannot
// be mixed on a single CD.  There is no set standard today, but most
// people have agreed to use Mode 2 Form 1.
//
...
// If the DiskOffset is used, then we are assuming a LBA.  However, CdAddress
// provides the flexibility of accessing the device using MSF or LBA.
//
// If the high DWORD is clear, then we use the DiskOffset at LBA.  If not, then
// take the MSF address.  This allows backward compatibility since the MMC command
// only uses 4 bytes, and the high word is never used.
//
 ...
 *
 *  -------------------------------------------------------------
 *  1/26/2006 - Adding functionaality
 *
 *  SCSI MMC command READ DISC INFORMATION returns the structure
 *  defined as DISC_INFORMATION in the Scsi.h.  This sturcture
 *  contains information necessary for determining information
 *  about the disc that is not found in the CDROM_DISCINFO output
 *  originally specified by this function.
 *
 *  We don't want to remove backward compatibility, so we are
 *  adding an optional input parameter to specify which structure
 *  to return.
 *
 *  If the input parameter is NULL, then existing behavior is
 *  expected.  Given DISCINFO_IN as the input parameter, the
 *  correct structure to return will be based on the nOutput
 *  value.
 *  -------------------------------------------------------------
 *
 */

enum DISC_INFO { DI_NONE,           // Returns true from ATAPI.
                 DI_SCSI_INFO       // Returns DISC_INFORMATION
               };
...
typedef struct _CD_MODESENSE_HEADER
{
    UCHAR   cdmsh_DataLenHi;
    UCHAR   cdmsh_DataLenLo;
    UCHAR   cdmsh_medium;
    UCHAR   cdmsh_rsvd1;
    UCHAR   cdmsh_rsvd2;
    UCHAR   cdmsh_rsvd3;
    UCHAR   cdmsh_rsvd4;
    UCHAR   cdmsh_rsvd5;
} CD_MODESENSE_HEADER, * PCD_MODESENSE_HEADER;

// CD Read Error Recovery Parameter page - returned from MODE SENSE

typedef struct _CD_MODESENSE_READERRPAGE_DATA
{
    UCHAR   cdmsrep_pageinfo;
    UCHAR   cdmsrep_pagelength;
    UCHAR   cdmsrep_mode;
    UCHAR   cdmsrep_retrycount;
    UCHAR   cdmsrep_rsvd1;
    UCHAR   cdmsrep_rsvd2;
    UCHAR   cdmsrep_rsvd3;
    UCHAR   cdmsrep_rsvd4;
} CD_MODESENSE_READERRPAGE_DATA, * PCD_MODESENSE_READERRPAGE_DATA;

typedef struct _CDVD_CAPABILITIES
{
    UCHAR   cdvdc_pageinfo;
    UCHAR   cdvdc_pagelength;
    UCHAR   cdvdc_readbits;
    UCHAR   cdvdc_writebits;
    UCHAR   cdvdc_cddaisrc;
    UCHAR   cdvdc_lockprevent;
    UCHAR   cdvdc_changer;
    UCHAR   cdvdc_reserved[10];
} CDVD_CAPABILITIES, *PCDVD_CAPABILITIES;

typedef struct _CDMODE_SENSE_INFO
{
    CD_MODESENSE_HEADER header;
    CDVD_CAPABILITIES cap;
} CDMODE_SENSE_INFO, *PCDMODE_SENSE_INFO;

typedef struct _CD_PARTITION_INFO
{
    BOOL fAudio;
    DWORD dwStartingLBA;
    USHORT LastValidTrackInPartition;

    USHORT NumberOfTracks;
    TRACK_INFORMATION2 TrackInformation[1];
} CD_PARTITION_INFO, *PCD_PARTITION_INFO;

typedef struct
{
    int cbSize;
    BOOL fPause;
} CD_PAUSE_POLLING, *PCD_PAUSE_POLLING;

#define READCAPS_DVD 0x08
