...
//  reset various definitions of static assert appearing in the source tree
#   if defined(CASSERT)
#       undef CASSERT
#   endif
#   if defined(C_ASSERT)
#       undef C_ASSERT
#   endif
#   if defined(COMPILETIME_ASSERT)
#       undef COMPILETIME_ASSERT
#   endif

//  get STATIC_ASSERT definition
#   include <static_assert.hxx>

//  redefine all other aliases for static assert
#   define CASSERT(x)              STATIC_ASSERT(x)
#   define C_ASSERT(x)             STATIC_ASSERT(x)

//  COMPILETIME_ASSERT needs to end with a semicolon
#   define COMPILETIME_ASSERT(x)   STATIC_ASSERT(x);
...
#ifndef ASSERTSZ
#ifdef DEBUG
#define ASSERTSZ(x, sz) ( (x) || (OutputDebugString(TEXT(sz)), ASSERT(0), 0) )
#else
...
// This happens by design in ATL projects, exempt them
