...
typedef enum _MINIDUMP_STREAM_TYPE {
    ...
    ceStreamLastStream          = ceStreamDiagnosisList,
...
struct _CEDUMP_SYSTEM_INFO_V2
{
    ...
    DWORD   PtrKData;               // Ptr to KData memory block
};

#define CEDUMP_SYSTEM_INFO_SIZE_V1 (sizeof (struct _CEDUMP_SYSTEM_INFO_V1))
#define CEDUMP_SYSTEM_INFO_SIZE_V2 (sizeof (struct _CEDUMP_SYSTEM_INFO_V2))

typedef struct _CEDUMP_SYSTEM_INFO_V2 CEDUMP_SYSTEM_INFO, *PCEDUMP_SYSTEM_INFO;
...
struct _CEDUMP_THREAD_CALL_STACK_FRAME_V1
{
    ULONG32 ReturnAddr;
    ULONG32 FramePtr;
    ULONG32 ProcessId;
...
struct _CEDUMP_THREAD_CALL_STACK_FRAME_V2
{
    ULONG32 ReturnAddr;
    ULONG32 FramePtr;
    ULONG32 VMProcessId;
    ULONG32 Flags;
    ULONG32 Params[4];
    ULONG32 ActvProcessId;
};

typedef struct _CEDUMP_THREAD_CALL_STACK_FRAME_V2 CEDUMP_THREAD_CALL_STACK_FRAME, *PCEDUMP_THREAD_CALL_STACK_FRAME;
...
#define CEDUMP_EXCEPTION_STACK_BUFFER_OVERRUN  0x10  // Bit 4 = ReportFault API called by Stack Buffer Overrun detection

struct _CEDUMP_EXCEPTION_STREAM_V1
{
    USHORT  SizeOfHeader;
...
struct _CEDUMP_EXCEPTION_STREAM_V2
{
    // V1 fields
    USHORT  SizeOfHeader;
    USHORT  SizeOfException;            // Size of _CEDUMP_EXCEPTION
    USHORT  SizeOfThreadContext;        // Size of _CEDUMP_ELEMENT_LIST
    USHORT  Flags;                  
    ULONG32 VMProcessId;                // Current process for ThreadId
    ULONG32 ThreadId;                   // Thread ID where exception occurred, or focus of CaptureDumpFileOnDevice
    ULONG32 OwnerProcessId;             // Owner process for ThreadId
    ULONG32 CaptureAPIVMProcessId;      // Current process for CaptureAPIThreadId
    ULONG32 CaptureAPIThreadId;         // Thread ID which made the call to CaptureDumpFileOnDevice
    ULONG32 CaptureAPIOwnerProcessId;   // Owner process for CaptureAPIThreadId

    // V2 fields - We need these entries because all handles are coming from these processes.
    ULONG32 ActvProcessId;              // Active process for ThreadId
    ULONG32 CaptureAPIActvProcessId;    // Active process for CAptureAPIThreadId

};

typedef struct _CEDUMP_EXCEPTION_STREAM_V2 CEDUMP_EXCEPTION_STREAM, *PCEDUMP_EXCEPTION_STREAM;

//
// STREAM TYPE: ceStreamBucketParameters.
//

typedef struct _CEDUMP_BUCKET_PARAMETERS_V1
{
    USHORT  SizeOfHeader;
    USHORT  __unusedAlignment;
    RVA     EventType;          
    ...
    ULONG32 OwnerVerLS;
} CEDUMP_BUCKET_PARAMETERS_V1, *PCEDUMP_BUCKET_PARAMETERS_V1;


typedef struct _CEDUMP_PARAM_DESCRIPTOR
{
    ULONG32	Type;
    ULONG32	Data;
}
CEDUMP_PARAM_DESCRIPTOR, *PCEDUMP_PARAM_DESCRIPTOR;

#define NUM_EX_PARAM_DESCRIPTORS 6

enum {
    BUCKET_PARAM_PROC_NAME = 1,
    BUCKET_PARAM_MOD_NAME,
    BUCKET_PARAM_TIMESTAMP,
    BUCKET_PARAM_FILE_VERSION,
    BUCKET_PARAM_OFFSET,
    BUCKET_PARAM_SIZE,
    BUCKET_PARAM_TYPE
};
...
typedef struct _CEDUMP_BUCKET_PARAMETERS_V2
{
    // V1 fields
    ...
    ULONG32 OwnerStamp;         // Time Stamp of owner application
    ULONG32 OwnerVerMS;         // File version of owner application
    ULONG32 OwnerVerLS;        // 1 if debug build of application

    // V2 changes to accomodate event specific params

    CEDUMP_PARAM_DESCRIPTOR ExParam[NUM_EX_PARAM_DESCRIPTORS];

} CEDUMP_BUCKET_PARAMETERS_V2, *PCEDUMP_BUCKET_PARAMETERS_V2;

typedef struct _CEDUMP_BUCKET_PARAMETERS_V2 CEDUMP_BUCKET_PARAMETERS, *PCEDUMP_BUCKET_PARAMETERS;


typedef struct _MINIDUMP_MEMORY_DESCRIPTOR _CEDUMP_MEMORY_DESRIPTOR_V1;

struct _CEDUMP_MEMORY_DESCRIPTOR_V2
{
    // V1 - Windows CE 5.0
    ULONG64 StartOfMemoryRange;
    MINIDUMP_LOCATION_DESCRIPTOR Memory;
...
#define CEDUMP_MEMORY_DESCRIPTOR_SIZE_V1 (sizeof (_CEDUMP_MEMORY_DESRIPTOR_V1))
#define CEDUMP_MEMORY_DESCRIPTOR_SIZE_V2 (sizeof (struct _CEDUMP_MEMORY_DESCRIPTOR_V2))

typedef struct _CEDUMP_MEMORY_DESCRIPTOR_V2 CEDUMP_MEMORY_DESCRIPTOR, *PCEDUMP_MEMORY_DESCRIPTOR;
...
typedef struct _CEDUMP_MEMORY_LIST
{
    USHORT  SizeOfHeader;
    USHORT  SizeOfEntry;
    ULONG32 NumberOfEntries;    // Number of CEDUMP_MEMORY_DESCRIPTOR
} CEDUMP_MEMORY_LIST, *PCEDUMP_MEMORY_LIST;


//
// STREAM TYPE: ceStreamProcessModuleMap
//
// Process Module Map entry
//
// This structure maps a ProcessID to a ModuleID and includes the process
// specific refcount for that module.
//

typedef struct _CEDUMP_PROCESS_MODULE_ENTRY
{
    ULONG32 ProcessID;
    ULONG32 ModuleID;
    ULONG32 RefCount;
} CEDUMP_PROCESS_MODULE_ENTRY, *PCEDUMP_PROCESS_MODULE_ENTRY;

//
// STREAM TYPE: ceStreamProcessModuleMap
//
// Process Module Map header 
//
// This header must be followed by
// (NumberOfEntries * CEDUMP_PROCESS_MODULE_ENTRY).
//

typedef struct _CEDUMP_PROCESS_MODULE_LIST
{
    USHORT SizeOfHeader;
    USHORT SizeOfEntry;
    ULONG32 NumberOfEntries;    // Number of CEDUMP_PROCESS_MODULE_ENTRY
} CEDUMP_PROCESS_MODULE_LIST, *PCEDUMP_PROCESS_MODULE_LIST;


//
// STREAM TYPE: ceStreamDiagnosisList
//


typedef struct _CEDUMP_DIAGNOSIS_LIST
{
    USHORT  SizeOfHeader;
    USHORT  SizeOfEntry;
    ULONG32 NumberOfEntries;    // Number of CEDUMP_DIAGNOSIS_DESCRIPTOR
} CEDUMP_DIAGNOSIS_LIST, *PCEDUMP_DIAGNOSIS_LIST;


typedef struct _CEDUMP_DIAGNOSIS_DESCRIPTOR
{
    ULONG32  Type;
    ULONG32  SubType;
    ULONG32  ProcessId;
    ...
    ULONG32  Severity;
    ULONG32  Confidence;
    MINIDUMP_LOCATION_DESCRIPTOR Description;
    MINIDUMP_LOCATION_DESCRIPTOR Data;
    ULONG32  pDataTemp;  // Temporary
} 
CEDUMP_DIAGNOSIS_DESCRIPTOR, *PCEDUMP_DIAGNOSIS_DESCRIPTOR;


typedef struct _CEDUMP_HEAP_ITEM_DATA
{
    ULONG32 ProcessId;
    ULONG32 pHeap;
    ULONG32 pMem;
    ...
    ULONG32 dwFlags;
    ULONG32 cbSentinels;
    ULONG32 pBeginTruncateOffset;
    ULONG32 pEndTruncateOffset;
    MINIDUMP_LOCATION_DESCRIPTOR Item;
}
CEDUMP_HEAP_ITEM_DATA, *PCEDUMP_HEAP_ITEM_DATA;


typedef struct _CEDUMP_HEAP_CORRUPTION_DATA
{
    CEDUMP_HEAP_ITEM_DATA  CorruptItem;
    CEDUMP_HEAP_ITEM_DATA  PreviousItem;
} 
CEDUMP_HEAP_CORRUPTION_DATA, *PCEDUMP_HEAP_CORRUPTION_DATA;


typedef struct _CEDUMP_HEAP_INFO
{
    ULONG32 pProcess;
    ULONG32 ProcId;
    ULONG32 ModId;
...
CEDUMP_HEAP_INFO, *PCEDUMP_HEAP_INFO;


typedef struct _CEDUMP_PROC_MEMINFO
{
    ULONG32 cbSize;
    ULONG32 procId;
    ULONG32 nHeaps;
    ...
    ULONG32 cbHeap;
    ULONG32 nPageTypes;
    ULONG32 pageCount[NUM_PAGETYPES];
}
CEDUMP_PROC_MEMINFO, *PCEDUMP_PROC_MEMINFO;


typedef struct _CEDUMP_SYS_MEMINFO
{
    ULONG32 cbSize;
    ULONG32 pageFreeCount;
    ULONG32 cpNeed;
    ULONG32 cpLowThreshold;
    ULONG32 cpCriticalThreshold;
    ULONG32 cpLowBlockSize;
    ULONG32 cpCriticalBlockSize;
    ULONG32 nHeaps;
    MINIDUMP_LOCATION_DESCRIPTOR HeapInfo;
    ULONG32 nProcs;
    CEDUMP_PROC_MEMINFO ProcMemInfo[];
}
CEDUMP_SYS_MEMINFO, *PCEDUMP_SYS_MEMINFO;
