...
#define SETFNAME(name)          LPCTSTR pszFname = (name); DBG_UNREFERENCED_LOCAL_VARIABLE(pszFname)
...
typedef struct _ActivityTimer_tag {
    ...
    HANDLE      hevAutoReset;       // Auto reset event that use by PM.
...
PACTIVITY_TIMER ActivityTimerCreate(LPCTSTR pszName, DWORD dwTimeout, __in_z LPTSTR pszSources);
...
DWORD WINAPI PlatformMapPowerStateHint(DWORD, __out_ecount(dwBufChars) LPTSTR, DWORD dwBufChars); 
DWORD WINAPI PlatformSetSystemPowerState(LPCTSTR pwsState, BOOL fForce, BOOL fInternal);
DWORD WINAPI PlatformSendSystemPowerState(LPCWSTR pwsState, DWORD dwStateHint, DWORD Options);
