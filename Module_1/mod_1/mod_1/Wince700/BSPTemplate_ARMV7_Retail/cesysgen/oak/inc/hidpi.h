...
#ifdef UNDER_CE
#define HidP_IsSameUsageAndPage(u1, u2) \
    ( ((u1).Usage == (u2).Usage) && ((u1).UsagePage == (u2).UsagePage) )
#else
...
HidP_InitializeReportForID (
   ...
   IN       UCHAR                 ReportID,
   IN       PHIDP_PREPARSED_DATA  PreparsedData,
   __out_bcount(ReportLength) IN OUT   PCHAR                 Report,
...
HidP_GetUsages (
   ...
   IN       USAGE                UsagePage,
   IN       USHORT               LinkCollection, // Optional
   __out_ecount(*UsageLength) OUT      USAGE *              UsageList,
