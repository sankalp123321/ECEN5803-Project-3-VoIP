...
#define ANYBLT_WIDTH_LOOP_SIG 3
...
InstructionFetchAndDecode(
    const DWORD        * pdwPCode,
    PCodeInstruction_t * pInstr
    )
{
    pInstr->dwOpCode = *pdwPCode;

    pInstr->iArgCount  = GET_OPCODE_REGCOUNT(pInstr->dwOpCode);
    pInstr->iArgCount += GET_OPCODE_LITCOUNT(pInstr->dwOpCode);

    memcpy(&pInstr->dwArg1, pdwPCode + 1, pInstr->iArgCount * sizeof (DWORD));

    return (1 + pInstr->iArgCount);
}
