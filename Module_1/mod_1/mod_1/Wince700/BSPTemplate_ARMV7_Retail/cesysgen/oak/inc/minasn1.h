...
MinAsn1ParseExtensions(
    ...
    IN OUT DWORD *pcExt,
    __out_ecount_opt(*pcExt) OUT CRYPT_DER_BLOB rgrgExtBlob[][MINASN1_EXT_BLOB_CNT]
    );
...
MinAsn1ParseSignedDataCertificates(
    ...
    IN OUT DWORD *pcCert,
    __out_ecount_opt(*pcCert) OUT CRYPT_DER_BLOB rgrgCertBlob[][MINASN1_CERT_BLOB_CNT]
    );
...
MinAsn1ParseAttributes(
    ...
    IN OUT DWORD *pcAttr,
    __out_ecount_opt(*pcAttr) OUT CRYPT_DER_BLOB rgrgAttrBlob[][MINASN1_ATTR_BLOB_CNT]
    );
...
MinAsn1FindExtension(
    ...
    IN DWORD cExt,
    __out_ecount(cExt) IN CRYPT_DER_BLOB rgrgExtBlob[][MINASN1_EXT_BLOB_CNT]
    );
...
MinAsn1FindAttribute(
    IN PCRYPT_DER_BLOB pEncodedOIDBlob,
    IN DWORD cAttr,
    __out_ecount(cAttr) IN CRYPT_DER_BLOB rgrgAttrBlob[][MINASN1_ATTR_BLOB_CNT]
    );
...
//+-------------------------------------------------------------------------
//  Function: MinAsn1ParseNameValue
//
//  Parses an ASN.1 encoded NameValue attribute value.
//
//  Returns:
//      success -  > 0 => bytes skipped, length of the encoded NameValue attr 
//      failure -  < 0 => negative (offset + 1) of first bad tagged value
//--------------------------------------------------------------------------

// Encoded bytes including outer SEQUENCE tag and length octets
#define MINASN1_NAME_VALUE_ENCODED_IDX          1

// Name content bytes (BMPSTRING)
#define MINASN1_NAME_VALUE_NAME_IDX             2

// Type content bytes (INTEGER)
#define MINASN1_NAME_VALUE_TYPE_IDX             3

// Value content bytes (OCTETSTRING)
#define MINASN1_NAME_VALUE_VALUE_IDX            4

#define MINASN1_NAME_VALUE_BLOB_CNT             5

LONG
WINAPI
MinAsn1ParseNameValue(
    IN PCRYPT_DER_BLOB pAttrValueBlob,
    OUT CRYPT_DER_BLOB rgNameValueBlob[MINASN1_NAME_VALUE_BLOB_CNT]
    );

//+-------------------------------------------------------------------------
//  Function: MinAsn1EncCertList
//
//  Encodes an ASN.1 encoded PKCS #7 Signed Data Message. 
// Assumes:
//              -  the PKCS#7 message only contains certificates.
//              -  the PKCS #7 message is definite length encoded. 
//               - PKCS #7 version1.5, ie, not the newer CMS version.
//
//  Parameters:
//          pbBuf: if NULL, the function returns the required buffer size
//          cbBuf: size of output buffer
//          cCerts: number of encoded certs passed in rgCerts
//          rgCerts: array of CRYPT_DER_BLOBS, each pointing to an X.509 encoded cert
//  Returns:
//      
//      failure -  < 0
