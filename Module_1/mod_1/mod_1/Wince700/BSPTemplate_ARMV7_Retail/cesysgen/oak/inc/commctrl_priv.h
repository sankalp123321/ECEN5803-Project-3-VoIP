...
/// <summary> 
/// Optional creation struct for progress bar.
/// May be passed as the lpCreateParams member of the CREATESTRUCT.
/// </summary>
typedef struct tagPROGRESSBARCREATESTRUCT
{
    /// <summary>
    ...
    /// <summary>
    /// The layout to use for the progress bar control or NULL if the
    /// default layout should be used.
    /// </summary>
    LPCTSTR pszLayout;
} PROGRESSBARCREATESTRUCT, *PPROGRESSBARCREATESTRUCT;

/// <summary> 
/// Optional creation struct for track bar.
/// May be passed as the lpCreateParams member of the CREATESTRUCT.
/// </summary>
typedef struct tagTRACKBARCREATESTRUCT
{
    /// <summary>
    /// The size of this structure.
    /// </summary>    
    ...
    /// <summary>
    /// The layout to use for the track bar control or NULL if the
    /// default layout should be used.
    /// </summary>
    LPCTSTR pszLayout;
} TRACKBARCREATESTRUCT, *PTRACKBARCREATESTRUCT;
...
// BoxSelect specific styles used when creating the control
#define BSS_VERTICAL              0x0001    // Control is a row, not a column
#define BSS_MULTISELECT           0x0002    // Control allows multiple selection
#define BSS_IMAGELIST             0x0004    // Control uses an image list
#define BSS_VARIABLEWIDTH         0x0008    // Control will have variable-sized items 
                                            // (cannot be used with BSS_VERTICAL or BSS_IMAGELIST flag)
#define BSS_ALLOWNONE             0x0010    // None selection is allowed
                                            // (cannot be used with BSS_MULTISELECT flag)
#define BSS_SHAREIMAGELISTS       0x0020    // If set, the Image List is not deleted upon exit.
#define BSS_FLAT                  0x0040    // If set, will draw items with a flat look
#define BSS_TOOLTIP               0x0080    // Allows tooltips on the boxselect controls.

// BoxSelect specific messages here
#define BSM_GETCURSEL             (WM_USER + 600)
#define BSM_SETCURSEL             (WM_USER + 601)
#define BSM_GETSELCOUNT           (WM_USER + 602)
#define BSM_GETSELITEMS           (WM_USER + 603)
#define BSM_SETSEL                (WM_USER + 604)
#define BSM_GETSEL                (WM_USER + 605)
#define BSM_GETITEMCOUNT          (WM_USER + 606)
#define BSM_GETITEMTEXT           (WM_USER + 607)
#define BSM_GETITEMTEXTLENGTH     (WM_USER + 608)
#define BSM_ADDSTRING             (WM_USER + 609)
#define BSM_DELETESTRING          (WM_USER + 610)
#define BSM_SETIMAGELIST          (WM_USER + 611)
#define BSM_SETITEMINFO           (WM_USER + 612)
#define BSM_GETITEMINFO           (WM_USER + 613)
#define BSM_RESETCONTENT          (WM_USER + 614)
#define BSM_ENABLEITEM            (WM_USER + 615)

// BoxSelect specific notifications here
#define BSN_SELCHANGE             (NM_FIRST + 1)
...
#define SHSysGradient_RegValue TEXT("SHMirrorGradient")
...
// revert back to legacy updown control for this version and before
#define UPDOWN_LEGACY_MAJORVERSION 6

__inline DWORD GetClientProcessVersion()
{
    static DWORD g_dwProcessVersion = 0;

    if (g_dwProcessVersion == 0)
    {
        g_dwProcessVersion = HIWORD(GetProcessVersion(GetCurrentProcessId()));
    }

    return g_dwProcessVersion;
}
