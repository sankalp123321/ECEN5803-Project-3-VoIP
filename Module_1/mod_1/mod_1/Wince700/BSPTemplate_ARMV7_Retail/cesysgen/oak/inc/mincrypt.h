...
#define MINCRYPT_MAX_HASH_CONTEXT_LEN       400 // arbitrary limit.

#define MINCRYPT_MAX_HASH_LEN               64
#define MINCRYPT_SHA1_HASH_LEN              20
#define MINCRYPT_SHA_256_HASH_LEN           32
#define MINCRYPT_SHA_384_HASH_LEN           48
#define MINCRYPT_SHA_512_HASH_LEN           64
...
//   
//  HashCtx is used internally by the hashing functions.  It should remain
//  valid and unmodified by the caller until all usages of *phHash are
//  complete.
//
...
LONG
WINAPI
MinCryptCreateHashMemoryMTH(
    IN ALG_ID HashAlgId,
    IN OUT BYTE HashCtx[MINCRYPT_MAX_HASH_CONTEXT_LEN] ,
    OUT DWORD *phHash
    );


// This function is not thread safe. For thread safety, use the MTH version 
LONG
WINAPI
...
MinCryptUpdateHashMemory(
    ...
    IN DWORD hHash,
    IN DWORD cBlob,
    __in_ecount(cBlob) IN PCRYPT_DER_BLOB rgBlob
    );
    //+-------------------------------------------------------------------------
//  Hashes one or more memory blobs according to the Hash ALG_ID.
//
...
MinCryptVerifyCertificate(
    ...
    __out_ecount(cIssuerCert) IN CRYPT_DER_BLOB rgrgIssuerCertBlob[][MINASN1_CERT_BLOB_CNT],
    IN BOOL fTestRoot
    );
