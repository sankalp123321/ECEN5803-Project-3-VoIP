...
#else
#define WIN_IPV6_BASE_DEVICE_NAME L"IP60:"
#define WIN_IPV6_DEVICE_NAME      L"IP60:"
#endif // UNDER_CE
...
#ifdef UNDER_CE
    unsigned int AdvCurHopLimit;         // -1 means no change.
    unsigned int AdvReachableTime;       // Milliseconds, -1 means no change.
#endif
...
#ifdef UNDER_CE
    Info->AdvCurHopLimit = (unsigned int)-1;
    Info->AdvReachableTime = (unsigned int)-1;
#endif
}
...
#ifdef UNDER_CE
__inline int
IsDADStateValid(unsigned int State)
{
    return (State >= DAD_STATE_DEPRECATED);
}
#endif
