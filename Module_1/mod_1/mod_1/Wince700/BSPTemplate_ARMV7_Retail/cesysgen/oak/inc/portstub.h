...
//  Function stubs
//
inline __time32_t time (__time32_t *t) {
    SYSTEMTIME st;
    LARGE_INTEGER ft;
...
inline LONG RegQueryValue(HKEY hKey, LPCTSTR lpSubKey, __out_bcount(*lpcbValue) LPTSTR lpValue, PLONG lpcbValue) {
    HKEY hkey = NULL;
    LONG hr;
    ...
    hr = RegOpenKeyEx (hKey, lpSubKey, 0, KEY_QUERY_VALUE, &hkey);
    if (hr != ERROR_SUCCESS)
        return (hr == ERROR_INVALID_PARAMETER) ? ERROR_FILE_NOT_FOUND : hr;
...
inline LONG RegSetValue(  HKEY hKey,         // handle to key to set value for
  ...
  __in_bcount(cbData) LPCTSTR lpData,    // address of value data
...
inline LONG RegOpenKey(HKEY hKey, LPCTSTR lpSubKey, PHKEY phkResult) {
    LONG hr = RegOpenKeyEx (hKey, lpSubKey, 0, (KEY_READ | KEY_WRITE), phkResult);
    return (hr == ERROR_INVALID_PARAMETER) ? ERROR_FILE_NOT_FOUND : hr;
}

#define RegOpenKeyW RegOpenKey

inline LONG RegEnumKey(HKEY hKey, DWORD dwIndex, __out_ecount(cbName) LPTSTR lpName, DWORD cbName) {
    FILETIME ft;
...
inline DWORD GetShortPathName(
  ...
  __out_ecount(cchBuffer) LPTSTR lpszShortPath,  // pointer to a buffer to receive the
...
inline UINT GetDriveTypeW(LPCWSTR lpRootPathName) {
...
inline DWORD GetFullPathName(
  ...
  __out_ecount(nBufferLength) LPTSTR lpBuffer,     // pointer to path buffer
...
inline DWORD GetProfileString(
  ...
  __out_ecount(nSize) LPTSTR lpReturnedString,  // address of destination buffer
  DWORD nSize               // size of destination buffer
) {
    TCHAR szKeyName[MAX_PATH+1];
    HKEY hkey      = NULL;

    _tcscpy_s (szKeyName, MAX_PATH, TEXT("Profile\\"));
    if (lpAppName) {
        _tcsncat_s (szKeyName, _countof(szKeyName), lpAppName, _TRUNCATE);
    }
...
inline BOOL WriteProfileString(  LPCTSTR lpAppName, // pointer to section name
  ...
  LPCTSTR lpString   // pointer to string to write
    ...
    if (RegCreateKeyEx (HKEY_LOCAL_MACHINE, szKeyName, 0, NULL, 0, KEY_WRITE, NULL, &hkey, &dwDisp) == ERROR_SUCCESS) {
        if (! lpString) {
...
const WCHAR *lpSourceName) {
    ...
    HANDLE hEventLog;
    swprintf_s (szBuffer, _MAX_PATH-1, L"\\Temp\\%s - %s.log", lpUNCServerName ? lpUNCServerName : L"$localhost$", lpSourceName);
    if(0 == _wfopen_s (&hEventLog, szBuffer, L"a"))
    {
        return hEventLog;
    }
    return NULL;
...
inline BOOL GetComputerName(__out_ecount_opt(*pcchSize) PWSTR wszBuffer, __inout LPDWORD pcchSize) {
    WCHAR szBuffer[_MAX_PATH];
    DWORD dwType = REG_SZ;
    ...
    if (dwSize > 0) {
        szBuffer[(dwSize - 1) / sizeof(TCHAR)] = L'\0';
    } else {
        szBuffer[0] = L'\0';
    }
    ...
    *pcchSize = dwSize;
    memcpy (wszBuffer, szBuffer, dwSize * sizeof(TCHAR));
    return TRUE;
}
...
inline DWORD GetModuleFileNameCEA(
    ...
    WCHAR szBuffer[_MAX_PATH];
    if (! GetModuleFileNameW (hModule, szBuffer, sizeof(szBuffer) / sizeof(szBuffer[0])))
        return 0;
