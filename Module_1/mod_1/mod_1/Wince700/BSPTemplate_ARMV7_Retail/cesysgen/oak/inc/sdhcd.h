...
typedef enum _SD_SLOT_OPTION_CODE {
    ...
    SDHCDPowerControlHandler,       // set slot power control handler
...
typedef enum _SD_SLOT_EVENT {
    ...
    DeviceSuspended,    // Handle Suspend, stop thread processing.
    DeviceResumed       // Handle Resume, start thread processing.
...
#define SDHCDSetHCName(pHc, d)                  \
    (wcsncpy_s((pHc)->HostControllerName, _countof((pHc)->HostControllerName) - 1, d, _TRUNCATE))
...
typedef struct _SDHOST_API_FUNCTIONS {
    ...
    VOID (*pUpdateCurrentHCOwned)(PSDCARD_HC_CONTEXT  pHCContext, 
        DWORD               SlotIndex,
        PSD_BUS_REQUEST     pRequest);

    SD_API_STATUS (*pCheckHardware)(PSDCARD_HC_CONTEXT  pHCContext, 
        DWORD               SlotIndex,
        PSD_CARD_STATUS     pCardStatus);

    SD_API_STATUS (*pUpcallSetAdaptiveControl)(PSDCARD_HC_CONTEXT  pHCContext, 
        DWORD                           SlotIndex,
        PVOID   pAdaptiveControl);

    SD_API_STATUS (*pUpcallGetAdaptiveControl)(PSDCARD_HC_CONTEXT  pHCContext, 
        DWORD                           SlotIndex,
        PVOID   pAdaptiveControl);

    SD_API_STATUS (*pGetLocalAdaptiveControl)(PSDCARD_HC_CONTEXT  pHCContext, 
        DWORD                           SlotIndex,
        PVOID   pAdaptiveControl);

    SD_API_STATUS (*pFreeAllOutStandingRequests)(PSDCARD_HC_CONTEXT pHCContext, 
        DWORD                           SlotNumber);

    SD_API_STATUS (*pSynchronousSlotStateChange)(PSDCARD_HC_CONTEXT pHCContext, 
        DWORD                           SlotNumber, 
        SD_SLOT_EVENT                   Event);

    SD_API_STATUS (*pHandleResetDevice)(PSDCARD_HC_CONTEXT pHCContext, 
        DWORD                           SlotNumber);
...
// SDHCDUpdateCurrentHCOwned - Updates the HCOwned Request after it has been flushed
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
//          pRequest - request
// Output:
// Return:        
// Notes:   This function notifies the bus driver when the request has been replaced.
VOID SDHCDUpdateCurrentHCOwned(
    PSDCARD_HC_CONTEXT  pHCContext, 
    DWORD SlotIndex,
    PSD_BUS_REQUEST pRequest);

// SDHCDCheckHardware - Checks the Card Harware State
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
...
// Output:
// Return:  SD_API_STATUS
// Notes:   This function checks to see if the hardware state has changed
SD_API_STATUS SDHCDCheckHardware(
    PSDCARD_HC_CONTEXT  pHCContext, 
    DWORD SlotIndex,
    PSD_CARD_STATUS pCardStatus);

// SDHCDUpcallSetAdaptiveControl - Sets the adaptive control of the top level driver
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
//          pAdaptiveControl - adaptive control
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDUpcallSetAdaptiveControl(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber,
    PVOID   pAdaptiveControl);

// SDHCDUpcallGetAdaptiveControl - Gets the adaptive control from the top level driver
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
//          pAdaptiveControl - adaptive control
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDUpcallGetAdaptiveControl(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber,
    PVOID   pAdaptiveControl);

// SDHCDGetLocalAdaptiveControl - Gets the local adaptive control from the slot
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
//          pAdaptiveControl - adaptive control
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDGetLocalAdaptiveControl(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber,
    PVOID   pAdaptiveControl);

// SDHCDFreeAllOutStandingRequests - Frees all the outstanding requests
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDFreeAllOutStandingRequests(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber);

// SDHCDSynchronousSlotStateChange - synchronous slot state change request
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
//          Event
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDSynchronousSlotStateChange(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber, 
    SD_SLOT_EVENT                   Event);

// SDHCDHandleResetDevice
//                             
// Input:   pHCContext - host controller context   
//          SlotIndex
// Output:
// Return:  SD_API_STATUS
SD_API_STATUS SDHCDHandleResetDevice(
    PSDCARD_HC_CONTEXT pHCContext, 
    DWORD                           SlotNumber);
