...
extern HANDLE   CTEHeapCreate(char * Name);
extern BOOL     CTEHeapDestroy(HANDLE hHeap);
extern LPVOID   CTEHeapAlloc(HANDLE hHeap, DWORD Flags, DWORD Size);
extern BOOL     CTEHeapFree(HANDLE hHeap, LPVOID Ptr);
...
BOOL GetRegMultiSZValue (HKEY hKey, LPTSTR lpValueName, __out_bcount(dwSize) LPTSTR pData,
						 DWORD dwSize);

BOOL GetRegSZValue (HKEY hKey, LPTSTR lpValueName, __out_bcount_opt(dwSize) LPTSTR pData, DWORD dwSize);

BOOL GetRegBinaryValue (HKEY hKey, LPTSTR lpValueName, __out_bcount_opt(*pdwSize) LPBYTE pData, LPDWORD pdwSize);
...
BOOL
SetRegBinaryValue (HKEY hKey, LPTSTR lpValueName, __out_bcount(dwSize) LPBYTE pData, DWORD dwSize);

#define COMM_REG_KEY	TEXT("Comm\\")

DWORD
CXUtilGetProcAddresses(
	__in	PTCHAR	   tszDllName,
	__inout	HMODULE	  *phDll,
	...);
...
#define CXPORT_DEVICE_KEY L"Comm\\Devices\\Cxport"
// Name used when calling CreateFile
#define CXPORT_LEGACY_NAME L"CTE0:"


ULONG
CE_RtlNtStatusToDosError (__in NTSTATUS NTStatus);
