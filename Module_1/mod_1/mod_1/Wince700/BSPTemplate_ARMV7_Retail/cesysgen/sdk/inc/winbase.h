...
FORCEINLINE LONG InterlockedAnd_Inline (LONG volatile *pTarget, LONG mask)
{
    LONG lOldValue;
    do {
        lOldValue = *pTarget;
    } while (InterlockedCompareExchange (pTarget, lOldValue & mask, lOldValue) != lOldValue);
    return lOldValue;
}
#endif
...
FORCEINLINE LONG InterlockedOr_Inline (LONG volatile *pTarget, LONG mask)
{
    LONG lOldValue;
    do {
        lOldValue = *pTarget;
...
/*
    StrSafeCchLength should be used for new code. unsafe_tcslen should only be
    used for code where we would disable the deprecated api warning.
*/
#ifdef UNICODE
#define unsafe_tcslen   unsafe_wcslen
#define unsafe_lstrlen  unsafe_wcslen
#else
#define unsafe_tcslen   unsafe_strlen
#define unsafe_lstrlen  unsafe_strlen
#endif
__inline __checkReturn size_t __cdecl unsafe_wcslen(__in_z const wchar_t *psz)
{
    return wcslen(psz);
}

__inline __checkReturn size_t __cdecl unsafe_strlen(__in_z const char *psz)
{
    return strlen(psz);
}
...
int WINAPI lstrlenA(__in LPCSTR lpString);
int WINAPI lstrlenW(__in LPCWSTR lpString);
...
int WINAPI lstrlenW(__in LPCWSTR lpString)
{
    return ((lpString != NULL) ? wcslen(lpString) : 0);
}
#endif
...
#pragma warning(push)
#pragma warning(disable: 4201) // suppress c4201
...
#pragma warning(pop) // restore c4201
...
// OpenModuleFlags
#define OPENMODULE_OPEN_DEBUG            0x01
#define OPENMODULE_OPEN_LOADLIBRARY      0x02
#define OPENMODULE_OPEN_LOADKERNEL       0x04
#define OPENMODULE_OPEN_EXECUTE          0x08
#define OPENMODULE_OPEN_DATAFILE         0x10
#define OPENMODULE_ALLOW_SHADOW          0x20
#define OPENMODULE_OPEN_CAB              0x40
#define OPENMODULE_OPEN_ROMMODULE        0x80
#define OPENMODULE_OPEN_CATALOG          0x100
#define OPENMODULE_OPEN_INACCOUNT        0x200
#define OPENMODULE_SKIPPOLICY            0x400
