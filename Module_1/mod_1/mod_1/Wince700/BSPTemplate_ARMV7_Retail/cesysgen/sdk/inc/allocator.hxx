...
//  address_of function template can be used to get pointer to an object that overloads operator&
// 
template <typename T>
inline
T * address_of(T & obj)
...
template <typename T>
inline
T const * address_of(T const & obj)
{
    return reinterpret_cast<T const *>(&const_cast<char&>(reinterpret_cast<const volatile char&>(obj)));
}

template <typename T>
inline
T volatile * address_of(T volatile & obj)
{
    return reinterpret_cast<T volatile *>(&const_cast<char&>(reinterpret_cast<const volatile char&>(obj)));
}

template <typename T>
inline
T const volatile * address_of(T const volatile & obj)
{
    return reinterpret_cast<T const volatile *>(&const_cast<char&>(reinterpret_cast<const volatile char&>(obj)));
}
...
//
// The reference_allocator is used internally by containers to create an instance of a container
// object that references allocator from another instance. This allows using swap operation 
// within the container implementation regardless of actual allocator the container template
// is instanciated with (swap operation is supported only with allocators that don't maintain
// per-instance state).
// Since reference_allocator keeps a reference to another object, it may be used only in a context
// where its lifetime is guaranteed to be shorter than lifetime of referred object.
//
template <typename _Al>
class reference_allocator
{
public:
    template <typename _Al> friend bool operator==(const reference_allocator<_Al>&, const _Al&);
    template <typename _Al> friend bool operator==(const _Al&, const reference_allocator<_Al>&);
    
    reference_allocator(const _Al& allocator)
...
private:
    ...
    // operator= not needed - this declaration avoids a level 4 warning
    void operator=(const reference_allocator&);

private:
    const _Al& _Allocator;
};

template <typename _Al>
inline bool operator==(const reference_allocator<_Al>& _Left, const _Al& _Right)
    {return &_Left._Allocator == &_Right; }
  
template <typename _Al>
inline bool operator==(const _Al& _Left, const reference_allocator<_Al>& _Right)
    {return &_Left == &_Right._Allocator; }
