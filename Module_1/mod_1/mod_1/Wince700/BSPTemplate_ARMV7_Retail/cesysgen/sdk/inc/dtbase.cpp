...
#else // #ifndef IEMOBILE
            // alloca/_alloca is not safe, use _malloca instead
            ppInputs = (IUnknown **)_malloca(m_ulNumInputs * sizeof(IUnknown *));
            if( ppInputs == NULL )
            {
...
#else // #ifndef IEMOBILE
            // alloca/_alloca is not safe, use _malloca instead
            CDXTWorkInfoNTo1 *WIArray = (CDXTWorkInfoNTo1*)_malloca( sizeof(CDXTWorkInfoNTo1) *
                                                         ulNumBandsToDo );
            DWORD *TaskIDs = (DWORD*)_malloca( sizeof(DWORD) * ulNumBandsToDo );
            DXTMTASKINFO* TaskInfo = (DXTMTASKINFO*)_malloca( sizeof( DXTMTASKINFO ) *
                                                            ulNumBandsToDo );
            if ( WIArray == NULL || TaskIDs == NULL || TaskInfo == NULL)
            {
                _freea(WIArray);
                _freea(TaskIDs);
                _freea(TaskInfo);
...
#if (DXBT_DISCRETE != 0)
#error DXBT_DISCRETE should equal to 0
#endif
...
#else // #ifndef IEMOBILE
                // alloca/_alloca is not safe, use _malloca instead
                ULONG * aulInIndex = (ULONG *)_malloca(sizeof(ULONG) * m_ulMaxInputs);
                BYTE * aWeights = (BYTE *)_malloca(sizeof(BYTE) * m_ulMaxInputs);
                if (aWeights == NULL || aulInIndex == NULL)
                {
                    _freea(aWeights);
                    _freea(aulInIndex);
                    hr = E_OUTOFMEMORY;
