...
//  of samples required.
//
// the _alloca is obsoleted and we should prevent using them
#ifndef IEMOBILE
        ...
        if (SUCCEEDED(hr))
        {
            ...
            if( SUCCEEDED( hr ) )
            {
                ...
                else // if ((dwFlags & DXBOF_DITHER) || ((dwFlags & DXBOF_DO_OVER) && bSrcIsOpaque== 0))
                {
                    ...
                    {
                        CComPtr<IDirectDrawSurface> cpDDSrc = NULL;
                        ...
                        if (SUCCEEDED(hrDD))
                        {
                            CComPtr<IDirectDrawSurface> cpDDDest = NULL;
...
inline BOOL DXIsBadReadPtr( const void* pMem, UINT Size )
{
    ...
    // the IsBadXXXPtr is obsoleted and we should never ever use them
    return (NULL == pMem);
#endif // #ifndef IEMOBILE
#endif
}
