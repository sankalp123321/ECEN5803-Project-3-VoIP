...
// warning C4510: 'ce::list_impl::_ListNode<T>' : default constructor could not be generated
#pragma warning(disable: 4510)

// warning C4610: struct 'ce::list_impl::_ListNode<T>' can never be instantiated - user defined constructor required
#pragma warning(disable: 4610)

// warning C4512: 'ce::list_impl::_ListNode<T>' : assignment operator could not be generated
...
struct _ListNode
{
    T           _Value;         // _Value MUST be first field of _ListNode structure;
    _ListNode  *_Next, *_Prev;  
};
...
template <typename _Tr> class hash;
...
{
protected:
    typedef list_impl::_ListNode<T> _Node;
    typedef _Node*                  _NodePtr;
    
    typedef list<T, details::reference_allocator<_Al> > _TmpList;
    
    // friend all other instances of list
    template <typename T1, typename _Al1> friend class list;
    
    // friend hash
    template <typename _Tr> friend class details::hash;

public:
    typedef list_impl::iterator<T>                                          iterator;
    typedef list_impl::const_iterator<T>                                    const_iterator;
...
public:
    // assign
    bool assign(const list& Right)
    {
        _TmpList _Tmp(get_allocator());
        
        if(!_Tmp.insert(_Tmp.begin(), Right.begin(), Right.end()))
            return false;
    ...
    iterator insert(iterator Where, const _Val& Value)
    {
        ...
        if(!assign_object(NewNode->_Value, Value))
        {
            erase(NewNode);
            return end();
        }
...
public:    
    ...
    void splice(iterator _Where, list& _Right, iterator _First, iterator _Last)
    {    
...
protected:
    ...
    void _Splice(iterator _Where, list& _Right, iterator _First, iterator _Last, size_type _Count)
    {    
    ...
    void _Swap(list<T, _Allocator>& Right)
    {
        // Supported only if both lists use the "same" allocator,
        // not the same type but an "interchangeable" instance.
        // After the swap, items from one list will be deallocated
        // by the allocator of the other list and vice versa.
        // To remove this limiation, allocators would have to support
        // swap operation. 
        assert(Right.get_allocator() == get_allocator());
        
        _NodePtr _RightNext, _RightPrev;
        
        if(Right._Head->_Next != Right._Head)
        {
            assert(Right._Head->_Prev != Right._Head);

            _RightNext = Right._Head->_Next;
            _RightPrev = Right._Head->_Prev;
        }
        else
        {
            assert(Right._Head->_Prev == Right._Head);
            
            _RightPrev = _RightNext = _Head;
        }    
        ...
        if(_Head->_Next != _Head)
        {
            ...
            Right._Head->_Next = _Head->_Next;
            Right._Head->_Prev = _Head->_Prev;
            _Head->_Next->_Prev = _Head->_Prev->_Next = Right._Head;
        }
        else
        {
            assert(_Head->_Prev == _Head); 
            
            Right._Head->_Prev = Right._Head->_Next = Right._Head;    
        }
        
        _Head->_Prev = _RightPrev;
        _Head->_Next = _RightNext;
        _Head->_Prev->_Next = _Head->_Next->_Prev = _Head;
        
        std::swap(Right._Size, _Size);
    }

protected:
    // only _Next and _Prev, but not _Value, are valid for _Head
    _NodePtr        _Head;
