...
inline BOOL DXIsValidAddress(const void* lp, UINT nBytes, BOOL bReadWrite)
{
    ...
    UNREFERENCED_PARAMETER(bReadWrite);
    // IsBadXXXPtrs are obsoleted api, we should prevent using them
    return (lp != NULL);
...
#else // #ifndef IEMOBILE
        UINT cBytes = 0;
        // use safe int fucntions to prevent integer overflow/underflow
        // if safe int function failed, we should stop allocate memory
        if ( FAILED(UIntMult(nMax, cbElement, &cBytes))
             || FAILED(UIntAdd(cBytes, sizeof(CDXPlex), &cBytes))
             || (cBytes > SIZE_MAX) )
        {
            return NULL;
        }
        CDXPlex* p = (CDXPlex*) new BYTE[cBytes];
#endif // #ifndef IEMOBILE
        if (p == NULL)
            return NULL;
        p->nMax = nMax;
