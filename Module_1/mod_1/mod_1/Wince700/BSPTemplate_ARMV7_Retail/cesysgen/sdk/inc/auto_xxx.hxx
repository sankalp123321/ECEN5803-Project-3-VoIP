...
/**/    // This version will leak the managed resource if allocation of the ref count object fails.
/**/    
...
protected:
    ...
    void copy(const _ref_counting_t& p)
    {
        assert(m_pRefCount == NULL);
...
#ifdef AUTO_XXX_DEFAULT_ALLOC

// use default allocator, it delegates the allocation to LocalAlloc/LocalFree
// and so it's safe to use across modules (allocate in one DLL and deallocate in another)
//
typedef _ref_counting_t<ce::allocator> ref_counting;

#else

// instance of _ref_couting_t template using a free list allocator to minimize heap fragmentation
// note: singleton_allocator is not safe to use across modules, because it has static data members
// (which are per-module)
//
typedef _ref_counting_t<ce::singleton_allocator<ce::free_list<100, allocator>, sizeof(long)> > ref_counting;

// explicitly instantiate lock_factory for the singleton free list allocator which is used by smart_ptr
template<> ce::singleton<ce::free_list<100, ce::allocator>, sizeof(long)>::lock_factory 
    ce::singleton<ce::free_list<100, ce::allocator>, sizeof(long)>::_factory;
...
public:
    ...
    auto_xxx(T _Y)
        : x(_Y) 
    {
        if (!copy_traits::create())
        {
            // failed to create the copy_traits object,
            // release the resource and bail out
            if(x != _Invalid)
                (*fnClose)(x);
        ...
        : x(_Y.x) 
    {
        ...
        // You can instantiate auto_xxx with ce::ref_counting or ce::linked traits to support 
        // copy semantics.
        copy_traits::copy(_Y);
    }
    ...
    auto_xxx& operator=(const auto_xxx& _Y)
    {
        auto_xxx(_Y).swap(*this);
        
        return (*this);
    }
    
    auto_xxx& operator=(const T& _Y)
    {
        if(copy_traits::release() && x != _Invalid)
            (*fnClose)(x);
...
//  comparison operators for auto_xxx
//

template<class T, class Fn, Fn fnClose, T _Invalid, class copy_traits>
inline
bool operator==( auto_xxx<T, Fn, fnClose, _Invalid, copy_traits> const & lhs
               , auto_xxx<T, Fn, fnClose, _Invalid, copy_traits> const & rhs)
{
...
//  the following comparison operators are here to avoid the default convertion from the raw pointer to the smart pointer
//
...
// Types with names starting with auto_ have no copy semantics.
