...
// This structure is used instead of POWER_BROADCAST when the broadcast
// type is PBT_POWERSTATUSCHANGE.  CE versions earlier than 7.0 set the
// Length and NewSystemPowerState fields to zero.
//
typedef struct _POWER_BROADCAST_POWER_STATUS_CHANGE  {
    DWORD       Message;                // PBT_POWERSTATUSCHANGE 
    DWORD       Flags;                  // Unused
    DWORD       Length;                 // Byte count of data starting at SystemPowerStateName (2)
    WORD        NewSystemPowerState;    //  AC_LINE_OFFLINE, AC_LINE_ONLINE,  AC_LINE_BACKUP_POWER or AC_LINE_UNKNOWN
} POWER_BROADCAST_POWER_STATUS_CHANGE, *PPOWER_BROADCAST_POWER_STATUS_CHANGE;
