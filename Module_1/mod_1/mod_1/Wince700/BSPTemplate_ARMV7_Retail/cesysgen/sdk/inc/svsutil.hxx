...
#define SVSUTIL_ARRLEN(array)           (_countof(array))
...
extern int      g_ThreadPriority256;
...
int svsutil_AssertBroken (TCHAR const*const lpszFile, int iLine);
...
// Local critical section that uses SVSSynch helper rather than raw CRITICAL_SECTION
class SVSLocalCriticalSectionSync {
    SVSSynch *m_pSync;

public:
    SVSLocalCriticalSectionSync(SVSSynch *pSync) {
        m_pSync = pSync;
        pSync->Lock();
    }

    ~SVSLocalCriticalSectionSync(void) {
        m_pSync->Unlock();
    }
};
...
public:
    SVSHandle   AllocHandle (void *a_pvData) {
        ...
        if (iFreeNdx == -1) {
            ...
            iFreeNdx = iArraySize;
            (*pArray)[iArraySize].u.iNextFree = -1;
        ...
        iFreeNdx = psHandle->u.iNextFree;

        psHandle->u.pvData = a_pvData;
    ...
    void        *CloseHandle (SVSHandle h) {
        ...
        void *pvData = psHandle->u.pvData;

        SVSUTIL_ASSERT (! psHandle->fFree);

        psHandle->u.iNextFree = iFreeNdx;
        psHandle->fFree     = TRUE;
        iFreeNdx = iNum;
    ...
    void        *ReAssociate (SVSHandle h, void *pvData2) {
        ...
        void *pvResult = (*pArray)[iNum].u.pvData;

        (*pArray)[iNum].u.pvData = pvData2;
...
public:
    ...
    {
        unsigned int uiTotalSize = sizeof(SVSPrivateHandle)*a_iMaxArraySize;

        // check for invalid parameter or integer overflow
        if ((a_iMaxArraySize<=0) || (uiTotalSize<(unsigned int)a_iMaxArraySize))  {
            pArray = NULL;
            iMaxArraySize   = 0;
        }
        else {
            pArray      = (SVSPrivateHandle *)g_funcAlloc (uiTotalSize, g_pvAllocData);
...
#define SVSUTIL_LINK_MAX_SEGMENT_SIZE    0x100000
...
public:
    ...
    SVSLinkManager(DWORD dwSizeOfEntry, SVS_PFN_DELETELINK pDeleteFcn=NULL, DWORD dwSegmentSize = SVSUTIL_LINK_LIST_INITIAL) {
        ...
        if (dwSizeOfEntry > SVSUTIL_LINK_MAX_SEGMENT_SIZE) {
            SVSUTIL_ASSERT(0);
            return;
        }
...
protected:
    ...
    SVSLinkManager          m_workerThreadEventList; // Linked list containing event HANDLE's that we are to fire.
    SVSThreadBlock          *m_pThreadBlockHead;     // Sorted list of threads to start, and when.
    SVSLinkManager          m_ThreadHandleList;      // List of all Thread Handles.  Used to syncorize shutdown.
    ...
    BOOL CreateNewWorkerThread(void) {
        ...
        pHandle = (HANDLE*)m_ThreadHandleList.AllocEntry();
        if (pHandle==NULL)
            goto done;
        ...
        if (m_stackSize)
            hThread = CreateThread (NULL, m_stackSize, SVSThreadPoolWorkerThread, pCntx, STACK_SIZE_PARAM_IS_A_RESERVATION, NULL);
        else
...
#ifdef UNDER_CE
        if (g_ThreadPriority256 > -1) {
            CeSetThreadPriority(hThread, g_ThreadPriority256);
        }
#endif
        ...
        *pHandle = hThread;
        ...
        if (fRet == FALSE) {
            if (pHandle) {
                m_ThreadHandleList.RemoveEntry(pHandle);
            }
    ...
    void EndWorkerThread(HANDLE *phThread, HANDLE hLocalEvent, HANDLE *phLocalInList) {
        Lock();

        phThread = (HANDLE*)m_ThreadHandleList.FindEntry(phThread);
        if (phThread) {
            CloseHandle(*phThread);
            m_ThreadHandleList.RemoveEntry(phThread);
        }
        else {
        ...
        if (phLocalInList) {
            ...
            BOOL f =
#endif
            m_workerThreadEventList.RemoveEntry(phLocalInList);

#if defined (SVSUTIL_DEBUG_ANY)
            if(f) 
            SVSUTIL_ASSERT(hLocalEvent != NULL);
#endif
        }
    ...
    void Worker(HANDLE *phThread) {
        ...
        // pListHandle points to hLocalEvent in m_workerThreadEventList if
        // it is in the list and needs removal, NULL if hLocalEvent isn't in list
        HANDLE         *pListHandle = NULL;
        ...
        if (hLocalEvent==NULL) {
            ...
            EndWorkerThread(phThread,NULL,NULL);
            return;
        }
        ...
        while (m_fRunning) {
            ...
            if (fTimer) {
                ...
                if (!m_fRunning)
                    ...
                    // m_workerThreadEventList is treated for the most part like
                    // a stack, in that we manually add entries to beginning of the
                    // list and remove from end first.  However, we use the
                    // SVSLinkManager structure because the worker may terminate
                    // in such a way that we need to remove its thread from
                    // the middle of the list.
                    pListHandle = (HANDLE*)m_workerThreadEventList.AllocEntry();
                    if (pListHandle) {
                        *pListHandle = hLocalEvent;
                        fSuccess = TRUE;
                    }
                    else
                        fSuccess = FALSE;
            ...
            if (WAIT_TIMEOUT == WaitForSingleObject (hWaitEvent, ulTimeOut)) {
                // If we're not the timer thread then we're done executing (maybe)...
                if (! fTimer) {
                    Lock();
                    if (WAIT_TIMEOUT == WaitForSingleObject(hWaitEvent,0)) {
                        // Make sure that another worker did not set hEvent immediately
                        // after we timed out.  In this case (standard) they did not, so exit.
                        EndWorkerThread(phThread,hLocalEvent,pListHandle);
                        Unlock();
                        return;
                    }
                    Unlock();

                    // goto top of while loop, event has been signaled
                    continue;
                }
            } else if (! fTimer) {
            ...
            if (m_cThreadsIdle) {
                if (! m_workerThreadEventList.IsEmpty()) {
                    HANDLE *pEvent = (HANDLE*) m_workerThreadEventList.GetFirstEntry();
                    SetEvent(*pEvent);
                    m_workerThreadEventList.RemoveEntry(pEvent);
                }
            }
...
public:
    ...
    SVSThreadPool (unsigned long ulMaxThreads = SVS_THREAD_POOL_DEFAULT_MAX_THREADS, 
               long stackSize = 0) :
               m_workerThreadEventList(SVS_THREAD_POOL_DEFAULT_MAX_THREADS) ,
               m_ThreadHandleList(sizeof(HANDLE))
    {
    ...
    ~SVSThreadPool (void) {
        ...
        SVSUTIL_ASSERT (m_pThreadBlockHead == NULL);
        SVSUTIL_ASSERT (m_ThreadHandleList.IsEmpty());
...
protected:
    ...
    void WakeupAllWorkerThreads(void) {
        ...
        HANDLE *pWorkerWakeupEvent = (HANDLE *)m_workerThreadEventList.GetFirstEntry();

        while (pWorkerWakeupEvent) {
            // Wake up each worker thread.
            SetEvent(*pWorkerWakeupEvent);
            pWorkerWakeupEvent = (HANDLE*)m_workerThreadEventList.GetNext();
        }
    }
...
#define MAX_FAST_SHUTDOWN_ATTEMPTS    25
    ...
    void WaitOnThreadExitShutdown(void) {
        ...
        for (;;) {
            ...
            m_ThreadHandleList.RemoveEntriesFiltered((SVS_PFN_DELETELINK_FILTER)IsThreadRunning,NULL);
            // Can exit this loop only when all threads have been shut-down.
            fBreak = m_ThreadHandleList.IsEmpty();
            Unlock();
...
public:
    ...
    BOOL AllocMem(unsigned int uiRequestedBytes)  {
        ...
        if(!pBuffer) {
            ...
            // integer overflow
            unsigned int uiNewSize = uiAlloc+uiBufSize;
            if (uiNewSize<uiAlloc)
                return FALSE;

            unsigned char *pTempBuffer = (unsigned char*) g_funcRealloc(pBuffer,uiNewSize,g_pvAllocData);
            if (!pTempBuffer)
                return FALSE;
    ...
    BOOL Append(const CHAR *pAppend, unsigned int uiLen) {
        unsigned int cbBuffer = uiLen + 1;
        if (cbBuffer < uiLen || cbBuffer < 1) // Check for integer overflow
            return FALSE;

        if(!AllocMem(cbBuffer))
            return FALSE;


        PREFAST_SUPPRESS(419,"AllocMem will return FALSE if our buffer is not large enough below");
...
public:
    ...
    BOOL Encode(const WCHAR *szData) {
        ...
        while (*pTrav) {
            for (i = 0; i < g_svsutilNumXmlEntities; i++) {
                if (g_svsutilXmlCharacters[i] == *pTrav) {
                    StringCbCopyW((WCHAR*)(pBuffer+uiNextIn), uiBufSize-uiNextIn, g_svsutilEntityReferences[i]);
...
// to update internal tables of InterfaceMapper.  Typically a service will
// call this during its IOCTL_SERVICE_NOTIFY_ADDR_CHANGE processing.
...
#ifdef _WS2DEF_ // _WS2DEF_ is needed to get PIP_ADAPTER_ADDRESSES
#include <iphlpapi.h> // For PIP_ADAPTER_ADDRESSES
...
inline void svsutil_EatChar(__inout_z CHAR* szInput, CHAR ch)
{
    CHAR const* szRead = szInput;
    LPSTR szWrite = szInput;
...
#define SVSUTIL_MAX_EVENT_FORMAT_LEN     1000
...
#ifdef _PRIV_WINBASE_H__
// We must add this _PRIV_WINBASE_H__ check because SVSSimpleLog relies upon
// IsSystemFile() for added security.  This API is not available to the SDK, but only
// on builds with WINCEOEM=1 set.

//
// Simple roll-over based logging that outputs raw data to a log file.  Presently
// does not do time/date formatting, though it could easily be extended to do this.
//

#define SVS_LOGFILE_OLD_POSTFIX         L".old"     // Post-pend this to the old log file
#define SVS_LOGFILE_DEFAULT_SIZE        32768       // Default max size of logs
#define SVS_LOGFILE_ABSOLUTE_MAX_SIZE   0x7FFFFFFF  // Absolute largest we'll let log file be.

class SVSSimpleLog {
private:
    WCHAR    m_szLogFile[MAX_PATH];    // Name of file we're writing log to
    WCHAR    m_szOldLogFile[MAX_PATH]; // Name of old file
    HANDLE   m_hLog;                   // File handle to the log
    DWORD    m_dwMaxLogSize;           // Maximum size to allow the log to grow to
    DWORD    m_dwCurrentSize;          // Current size of the log file.
    ...
    BOOL OpenLogFile(void) {
        if (m_hLog != INVALID_HANDLE_VALUE) {
            // A log handle is open already.
            return TRUE;
        }

        if (m_dwMaxLogSize == 0) {
            // We're configured not to allow logging (0 max size).
            return FALSE;
        }

        m_hLog = CreateFile(m_szLogFile, GENERIC_READ | GENERIC_WRITE, FILE_SHARE_READ, 0, OPEN_ALWAYS, FILE_ATTRIBUTE_NORMAL, 0);
        if (INVALID_HANDLE_VALUE == m_hLog) {
            CloseLogFile();
            return FALSE;
        }

        m_dwCurrentSize = GetFileSize(m_hLog,NULL);
        if (0xFFFFFFFF == m_dwCurrentSize) {
            CloseLogFile();
            return FALSE;
        }

        // Move write pointer to next place in log (so we pickup where we left off.
        if (m_dwCurrentSize != 0) {
            if (0xFFFFFFFF == SetFilePointer(m_hLog,m_dwCurrentSize,NULL,FILE_BEGIN)) {
                CloseLogFile();
                return FALSE;
            }
        }
    ...
    void ZeroLogFields(void) {
        m_hLog          = INVALID_HANDLE_VALUE;
        m_dwMaxLogSize  = 0;
        m_dwCurrentSize = 0;
        m_szLogFile[0]  = m_szOldLogFile[0] = 0;
    }
...
public:
    ...
    // NOTE: Trusted processes should ensure that non-trusted processes cannot
    // specify the log file information, as this would allow system files to be overwritten.
    BOOL LogConfigure(const WCHAR *szLogFile, const WCHAR *szLogFileOld, DWORD dwMaxLogSize=SVS_LOGFILE_DEFAULT_SIZE) {
        CloseLogFile();
        ZeroLogFields();

        if (dwMaxLogSize > SVS_LOGFILE_ABSOLUTE_MAX_SIZE) {
            // Log is too big
            return FALSE;
        }

        if ((NULL == szLogFile)    || (szLogFile[0] == 0)    ||
            (NULL == szLogFileOld) || (szLogFileOld[0] == 0) || (dwMaxLogSize == 0)) {
            // Log is not going to be used.  Stop init now.
            return FALSE;
        }

        if (((wcslen(szLogFile)+1)    > SVSUTIL_ARRLEN(m_szLogFile)) ||
            ((wcslen(szLogFileOld)+1) > SVSUTIL_ARRLEN(m_szOldLogFile)))
        {
            return FALSE;
        }

        StringCchCopyW(m_szLogFile, SVSUTIL_ARRLEN(m_szLogFile), szLogFile);
        StringCchCopyW(m_szOldLogFile, SVSUTIL_ARRLEN(m_szOldLogFile), szLogFileOld);

        // Only set m_dwMaxLogSize once initial file name checks are performed.
        m_dwMaxLogSize = dwMaxLogSize;
        return TRUE;
    }

    BOOL LogWrite(const BYTE *pbData, DWORD cbData) {
        SVSUTIL_ASSERT((cbData < m_dwMaxLogSize) || (m_dwMaxLogSize==0));

        if (! OpenLogFile())
            return FALSE;

        SVSUTIL_ASSERT_PREFAST(m_dwCurrentSize < SVS_LOGFILE_ABSOLUTE_MAX_SIZE); // Initialization checks this.

        // Rolling log file over
        if (m_dwCurrentSize + cbData > m_dwMaxLogSize) {
            if (! LogDoRollover())
                return FALSE;
        }

        SVSUTIL_ASSERT(m_hLog != INVALID_HANDLE_VALUE);

        DWORD dwWritten;
        if (! WriteFile(m_hLog,pbData,cbData,&dwWritten,NULL) || (dwWritten != cbData))
            return FALSE;

        m_dwCurrentSize += cbData;
        return TRUE;
    }

    // Forces current log to be closed, copied to old file, and a new current log is created.
    BOOL LogDoRollover(void) {
        CloseLogFile();
        // Not much we can do if delete/move files fails, so don't bother checking error codes.
        DeleteFile(m_szOldLogFile);
        MoveFile(m_szLogFile,m_szOldLogFile);
        // Now reopen the file
        return OpenLogFile();
    }
};
...
// Reads in a GUID from a WCHAR*
inline BOOL svsutil_ReadGuidW(const WCHAR *szGuid, GUID *pGuid, BOOL fHasBraces) {
    WCHAR const * const szFormat = fHasBraces ? L"{" SVSUTIL_GUID_FORMAT_W L"}" : SVSUTIL_GUID_FORMAT_W;

    DWORD data1, data2, data3;
    DWORD data4[8];

    if (11 != swscanf_s(szGuid, szFormat,
...
inline BOOL svsutil_DWORDtoBOOLErrCode (const DWORD errorCode)
{
...
// NOTE: If bReturnValue == FALSE and  GetLastError == NO_ERROR
// this function asserts, and returns ERROR_INTERNAL_ERROR
inline DWORD svsutil_BOOLtoDWORDErrCode (const BOOL bReturnValue)
{
...
// an SVS_PFN_BLOCK_UNTIL_EVENT will block until an event has been signalled,
// while potentially performing some other task.
typedef DWORD (SVS_PFN_BLOCK_UNTIL_EVENT)(const HANDLE h);

// Usually used as a helper function, will run lpStartRoutine, block until it
// completes using the lpBlockTillEventSignaled to perform the blocking and
// then return the value of the thread.
inline DWORD svsutil_RunFunctionOnThreadAndBlockUntilComplete(LPTHREAD_START_ROUTINE lpStartRoutine,LPVOID lpParamater, SVS_PFN_BLOCK_UNTIL_EVENT lpBlockTillEventSignaled)
{
    const HANDLE hThread = CreateThread(NULL,NULL,lpStartRoutine,lpParamater,0,NULL);
    if (hThread == NULL)
    {
        return GetLastError();
    }
    ...
    const DWORD ret = lpBlockTillEventSignaled(hThread);
    ...
    DWORD exitCode=0;
    const BOOL bRet = GetExitCodeThread(hThread,&exitCode);
    if (bRet != TRUE)
    {
        CloseHandle(hThread);
        return ERROR_INTERNAL_ERROR;
...
// Implementation of SVS_PFN_BLOCK_UNTIL_EVENT that only waits.
inline DWORD svsutil_WaitUntilEventSignalled(const HANDLE hObject)
{
    const DWORD ret = WaitForSingleObject(hObject,INFINITE);
...
inline DWORD svsutil_RunFunctionOnThreadAndWaitUntilComplete(LPTHREAD_START_ROUTINE lpStartRoutine, LPVOID lpParamater)
{
    return svsutil_RunFunctionOnThreadAndBlockUntilComplete(lpStartRoutine,lpParamater,svsutil_WaitUntilEventSignalled);
}

// Convert to and from base64 encodings.
